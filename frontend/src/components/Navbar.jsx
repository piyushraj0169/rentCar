...
const Navbar = () => {
    ...
    return (
        <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all">
            ...
            <Link to="/">
                <h1 className="text-xl font-bold cursor-pointer">CarRent</h1>
            </Link>
            ...
            <div className={`${open ? 'flex' : 'hidden'} absolute top-[60px] left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden z-50`}>
                <Link to="/" className="block w-full py-2" onClick={() => setOpen(false)}>Home</Link>
                <Link to="/about" className="block w-full py-2" onClick={() => setOpen(false)}>About</Link>
                <Link to="/contact" className="block w-full py-2" onClick={() => setOpen(false)}>Contact</Link>
                <Link to="/cars" className="block w-full py-2" onClick={() => setOpen(false)}>Cars</Link>
